...
let comparePage page1 page2 =
    if List.contains (page1, page2) rules then -1
    elif List.contains (page2, page1) rules then 1
    else 0
...
let notUpdatesOk updates = not (updatesOk updates)
...
updates
    |> List.filter notUpdatesOk              // filter to only the incorrectly-ordered updates
    |> List.map (List.sortWith comparePage)  // sort each update according to the rules
    |> List.map middleElement                // take the middle element of each update
    |> List.sum                              // sum the middle elements for the solution
    |> printfn "%A"
