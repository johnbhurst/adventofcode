...
// This program works much the same as day12_1.fsx, but it goes backwards from E to all possible starts and finds the minimum.
...
let eligibleMoves (row, col) =
    let heightOk (row2, col2) =
        ...
        height1 - height2 <= 1 || height1 < height2
...
findPaths [] [ (startPos, 0) ]
    |> List.filter (fun ((row, col), _) -> heightmap.[row].[col] = 'S' || heightmap.[row].[col] = 'a')
    |> List.map snd
    |> List.min
    |> printfn "%d"
