...
// push a list of items to ith stack of StackSet
let pushNItems i items stackSet =
    stackSet.items.[i] <- items @ stackSet.items.[i]
    stackSet
...
// pop n items from ith stack of StackSet
let popNItems n i stackSet = List.collect (fun _ -> [popItem i stackSet]) [1..n]

// move n items from src stack to dst stack in StackSet
let move n src dst stackSet =
    let items = popNItems n src stackSet
    pushNItems dst items stackSet
